...
fn main() {
    ...
    let mut MG = MfGraph::new(N * N + 2);
    let s = N * N;
    let t = N * N + 1;
    for i in 0..N {
        for j in 0..N {
            if 0 < i {
                MG.add_edge(i * N + j, (i - 1) * N + j, 1);
            }
            if i + 1 < N {
                MG.add_edge(i * N + j, (i + 1) * N + j, 1);
            }
            if 0 < j {
                MG.add_edge(i * N + j, i * N + (j - 1), 1);
            }
            if j + 1 < N {
                MG.add_edge(i * N + j, i * N + (j + 1), 1);
            }
            ...
            if ((i + j) % 2 == 0 && c[i][j] == 'B') || ((i + j) % 2 == 1 && c[i][j] == 'W') {
                MG.add_edge(s, i * N + j, INF);
            }
            else {
                MG.add_edge(i * N + j, t, INF);
            };
        }
    }
    println!("{}", (N * (N - 1) * 2) as isize - MG.flow(s, t));
}